...
func UpdateRsyslogServerConfig(configFilePath string) error {
	var command string
	newConfig := `
...
#Target="remote_host" Port="XXX" Protocol="tcp")`
	var allowedHostConfig string = "$AllowedSender TCP"

	connectionList, err := GetAllSSHConnection()
	if err != nil {
		return err
	}
	for _, connection := range connectionList {
		allowedHostConfig += ", " + connection.HostSSH
	}
	newConfig += "\n" + allowedHostConfig
...
func RestartRsyslogService() error {
